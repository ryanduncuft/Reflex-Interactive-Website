...
(async () => {
    ...
    const updatePageMetadata = (data, url) => {
        ...
        for (const [key, value] of Object.entries(metaTags)) {
            ...
            const selector = isOg ? `meta[property="${key}"]` : `meta[name="${key}"], link[rel="${key}"]`;
            const el = document.querySelector(selector);
            
            if (el) {
                el.setAttribute(isOg || key === 'canonical' ? 'content' : 'href', value);
            }
        }
    };
    ...
    const buildSearchIndex = async () => {
        ...
        try {
            ...
            if (Array.isArray(newsData)) {
                newsData.forEach(article => index.push({
        ...
        try {
            ...
            if (Array.isArray(gamesData)) {
                gamesData.forEach(game => index.push({
    ...
    const setupRevealAnimations = () => {
        ...
        requestAnimationFrame(() => {
            document.querySelectorAll('.reveal-on-load, .hero-entry').forEach((el, i) => {
                ...
                setTimeout(() => el.classList.add('visible'), baseDelay + i * CONFIG.CARD_HOVER_DELAY_MS);
            });
        });
    };
    ...
    const displayNewsDetails = async (id) => {
        ...
        try {
            ...
            document.querySelector('main').innerHTML = '<div class="text-center text-danger pt-5">Failed to load article. Please try again.</div>';
        } finally {
    ...
    const displayGameDetails = async () => {
        ...
        if (!id) {
            document.querySelector('main').innerHTML = '<div class="text-center text-danger pt-5">Game not found.</div>';
            return;
        }
        ...
        try {
            ...
            if (heroEl) {
                ...
                Object.assign(heroEl.style, { backgroundPosition: 'center', backgroundSize: 'cover' });
            }
            ...
            document.querySelector('main').innerHTML = '<div class="text-center text-danger pt-5">Failed to load game details. Please try again.</div>';
        }
    };
    ...
    const handleFormSubmission = async (e, form) => {
        ...
        try {
            ...
            form.innerHTML = `<p class="text-${success ? 'success' : 'danger'} fw-bold text-center">${message}</p>`;
